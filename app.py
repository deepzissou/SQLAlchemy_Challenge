...
#Set up the home page and list all available routes.
@app.route("/")
def welcome():
...
def precipitation():
    ...
    date1yr = latest_date - relativedelta(years=1)

    #Perform a query to retrieve the data and precipitation scores.
    data_from_final_year = session.query(Measurement.date, Measurement.prcp).filter(
        Measurement.date >= date1yr).all()
    ...
    all_precipication = []
    for date, prcp in data_from_final_year:
        if prcp != None:
            precip_dict = {}
...
def tobs():
    ...
    data_from_final_year = session.query(Measurement.date, Measurement.tobs).filter(
    ...
    #
    all_temperatures = []
    for date, temp in data_from_final_year:
        if temp != None:
            temp_dict = {}
            temp_dict[date] = temp
